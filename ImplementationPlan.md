...
- [x] **Scheduling mechanism: a scheduled GitHub Actions workflow** (`.github/workflows/refresh.yml`) — the CI runner is the scheduler, so no always-on worker/host is needed:
  ...
  - **Persist the rebuilt index** so the live app can consume it — publishes `vectorstore/index/` + `data/metadata.json` + `data/processed/` + `data/last_refresh.json` as a **workflow artifact** (swap in a data-branch commit / object-storage push for a persistent target), since the Actions runner is ephemeral and its filesystem is discarded after the run
...
- **`.github/workflows/refresh.yml`** — the scheduler itself: `schedule` cron `0 5 * * *` (10:30 IST) + `workflow_dispatch`, `concurrency` single-flight, checkout → setup-python 3.12 → install → playwright → `python -m scheduler.refresh`, then upload the index+corpus artifact on success / status artifact on failure.
